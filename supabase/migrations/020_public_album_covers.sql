-- Actualizar RPC get_public_album para incluir imágenes de portada y contraportada
...
BEGIN
  SELECT a.id, a.name, a.description, a.pack_size,
         cp.image_url AS portada_image_url,
         cc.image_url AS contraportada_image_url
  INTO v_album
  FROM albums a
  LEFT JOIN cover_templates cp ON cp.id = a.portada_template_id AND cp.is_active = true
  LEFT JOIN cover_templates cc ON cc.id = a.contraportada_template_id AND cc.is_active = true
  ...
  RETURN jsonb_build_object(
    'album', jsonb_build_object(
      ...
      'portada_image_url',       v_album.portada_image_url,
      'contraportada_image_url', v_album.contraportada_image_url
    ),
    'slots', (
